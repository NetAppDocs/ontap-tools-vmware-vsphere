...
a|
Click the profile that you want to modify from the profiles listed on the *VM Storage Policies* page and click *EDIT*.
|===
...
. Complete the pages in the Create VM Storage Policy wizard to set up a profile or to edit values to modify an existing profile.
+
