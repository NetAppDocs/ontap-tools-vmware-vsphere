...
[.lead]
You can perform an in-place upgrade to the latest release of ONTAP tools from your existing 9.7 setup following the instructions provided here.
...
NOTE: You can perform an in-place upgrade to the latest release of ONTAP tools only from your existing 9.7 version. When you upgrade to 9.7.1 release of ONTAP tools and you want to use vVols replication, then you will need to setup one more vCenter Server with ONTAP tools with Site Recovery Manager (SRM) installed.
 ...
 * After you upgrade to the latest OTV(9.10) release that has FIPS enabled but you have a older version of vCenter where FIPS is not supported, the deployment will still work.
 But if you upgrade your vCenter to the latest FIPS supported version and you have an earlier version of ONTAP tools, then the deployment will work only of FIPS is disabled on the vCenter.
====
