...
1. Create digest of the iso package and sign it to produce the signature:
openssl dgst -sha256 -sign code-signing/OTV-SRA-TGZ/OTV-SRA-TGZ-private.key -out GeneratedSinedFile19.iso.sig netapp-ontap-tools-for-vmware-vsphere-9.11-8413-upge.iso

2. Extract the public key from the code signing certificate you got issued from Entrust (csc-prod-OTV-SRA-TGZ.pem)
openssl x509 -in csc-prod-OTV-SRA-TGZ.pem -pubkey -noout > csc-prod-OTV-SRA-TGZ.pub
...
+
openssl dgst -sha256 -verify csc-prod-OTV-SRA-TGZ.pub -signature GeneratedSinedFile19.iso.sig netapp-ontap-tools-for-vmware-vsphere-9.11-8413-upgrade.iso
