...
run the below commands on the ONTAP CLI before you upgrade to enable the certificate based communication between ONTAP tools for VMware vSphere and ONTAP. The following commands do not create new roles, but they provide access to the APIs mentioned. 
