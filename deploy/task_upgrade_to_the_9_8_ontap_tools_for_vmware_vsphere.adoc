...
. From ONTAP tools for VMware vSphere 9.12 onwards, the authentication is done through certificate. You can either add CA signed certificate or a self-signed certificate. See, link:../configure/task_modify_storage_system.html[Modify storage systems] for instructions.
