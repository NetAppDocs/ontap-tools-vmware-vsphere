...
If you are going to deploy the latest release of ONTAP tools, you must see the topic link:../deploy/reference_port_requirements_for_vsc.html[Requirements for deploying the ONTAP tools]. The topic link:../deploy/task_upgrade_to_the_9_8_ontap_tools_for_vmware_vsphere.html[Upgrade to the latest release of ONTAP tools] has information on performing an in-place upgrade.
