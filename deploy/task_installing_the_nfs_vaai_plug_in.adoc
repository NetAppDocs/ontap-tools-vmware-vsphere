...
sidebar: sidebar
keywords: 
summary: 'You can install the NFS Plug-in for VMware vStorage APIs for Array Integration \(VAAI\) using the GUI of ONTAP tools for VMware vSphere.'
---
...
* You should have downloaded the installation package for the NFS Plug-in for VAAI (.vib) from .
+
...
* You should have enabled the vstorage option on the by using the vserver nfs modify -vserver vserver_name -vstorage enabled command.
