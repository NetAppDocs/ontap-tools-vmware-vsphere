...
link:concept_deploy_or_upgrade_ontap_tools.html[Deploy or upgrade ONTAP® tools for VMware vSphere]
...
a|
The upgrade procedure from an existing deployment of VSC to ONTAP tools depends on the version of VSC, and whether you have deployed ONTAP tools. The deployment workflows and upgrade section has more information. link:concept_installation_workflow_for_existing_users_of_ontap.html[Deployment workflow for existing users of ONTAP tools]
