---
permalink: manage/task_assign_qos_policies.html
sidebar: sidebar
keywords: 
...
NOTE: You cannot apply QoS settings at a datastore or virtual machine level for datastores that are provisioned on direct SVM's because ONTAP does not support QoS at SVM management level.
