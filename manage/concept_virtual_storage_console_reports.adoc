 ...
 ** Type of datastore: NFS, VMFS, and vVols
 ** Volume style
+
...
The Log Integrity Report shows the file integrity status. Log integrity is checked at scheduled intervals and the report is displayed in the Log Integrity Report tab. It also provides the status of the different log files that are being rolled over. The report shows if the file was tampered.
